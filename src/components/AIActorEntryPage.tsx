...
// 导入拆分后的组件
import { SortableAudioItem } from './AIActorEntryPage/Audio/SortableAudioItem';
import { AudioResourcePanel } from './AIActorEntryPage/Audio/AudioResourcePanel';
import { AudioBottomPanel } from './AIActorEntryPage/Audio/AudioBottomPanel';
import { SortableScriptCard } from './AIActorEntryPage/Script/SortableScriptCard';
import { SortableScriptItem } from './AIActorEntryPage/Script/SortableScriptItem';
import { BottomInputArea } from './AIActorEntryPage/Common/BottomInputArea';
import { TimeRangeInput } from './AIActorEntryPage/Common/TimeRangeInput';
import { SectionHeader } from './AIActorEntryPage/Common/SectionHeader';
import { SortableImageItem } from './AIActorEntryPage/Image/SortableImageItem';
import { ImageItemComponent } from './AIActorEntryPage/Image/ImageItemComponent';
import { SortableVideoItem } from './AIActorEntryPage/Video/SortableVideoItem';
import { VideoItemComponent } from './AIActorEntryPage/Video/VideoItemComponent';
import { SortableStoryboardItem } from './AIActorEntryPage/Storyboard/SortableStoryboardItem';
import { StoryboardList } from './AIActorEntryPage/Storyboard/StoryboardList';

// 导入类型定义
import type { ScriptCardProps } from './AIActorEntryPage/types';

// 导入工具函数
import { formatMillisecondsToTime } from './AIActorEntryPage/utils/formatTime';
...
function AIActorEntryPage() {
  ...
  const [backgroundType, setBackgroundType] = useState<string>(t('aiactoroEntry.image.background'));
  const [style, setStyle] = useState<string>(t('aiactoroEntry.image.ancient'));
  ...
  const handleVideoPreview = async () => {
    ...
    if (!sceneId && sceneId !== 0) {
      toast.error(t('aiactoroEntry.validation.selectSceneFirst'));
      return;
    }
    ...
    try {
      ...
      if (result.code === 0) {
        ...
        if (result.data?.downloadUrl) {
          ...
          toast.success(t('aiactoroEntry.messages.success.videoPreviewLoaded'));
        } else {
      ...
      toast.error(t('aiactoroEntry.messages.error.videoPreviewFailed', { error: (error as Error).message }));
    } finally {
  ...
  const handleCreateStoryboard = async (fileId: string, fileName: string, userPrompt?: string) => {
    ...
    try {
      ...
      toast.error(t('aiactoroEntry.messages.error.imageApplyFailed', { error: (error as Error).message }));
    }
  };
    ...
    if (over && active.id !== over.id) {
      ...
      if (oldIndex !== -1 && newIndex !== -1) {
        ...
        try {
          ...
          toast.error(t('aiactoroEntry.messages.error.sortingFailed', { error: (error as Error).message }));
        }
      }
    }
  };
  ...
  const handleDeleteStoryboard = async (itemId: string) => {
    try {
      ...
      const result = await response.json();
      if (result.code === 0) {
        toast.success(t('aiactoroEntry.messages.success.storyboardDeleted'));
        // 刷新分镜板列表
      ...
      toast.error(t('aiactoroEntry.messages.error.deleteFailed', { error: (error as Error).message }));
    }
  };
    ...
    if (over && active.id !== over.id) {
      ...
      if (oldIndex !== -1 && newIndex !== -1) {
        ...
        try {
          ...
          const result = await response.json();
          if (result.code === 0) {
            toast.success(t('aiactoroEntry.messages.success.storyboardSorted'));
          } else {
            throw new Error(result.message || '更新排序失败');
          }
        } catch (error) {
  ...
  const updateSceneName = async (sceneId: number, newSceneName: string) => {
    try {
      ...
      toast.error(t('aiactoroEntry.messages.error.sceneNameUpdateFailed', { error: (error as Error).message }));
      return false;
    }
  };
  ...
  const handleSaveSceneItem = async () => {
    ...
    if (startSeconds >= endSeconds) {
      toast.error(t('aiactoroEntry.validation.startTimeLessThanEnd'));
      return;
    }

    if (!editingSceneContent.trim()) {
      toast.error(t('aiactoroEntry.validation.enterContent'));
      return;
    }
    ...
    try {
      ...
      toast.error(t('aiactoroEntry.messages.error.saveFailed', { error: (error as Error).message }));
    }
  };
  ...
  const updateStoryboardTime = async (itemId: string, startTimeMs: number, endTimeMs: number) => {
    try {
      ...
      const result = await response.json();
      if (result.code === 0) {
        toast.success(t('aiactoroEntry.messages.success.timeUpdated'));
        // 刷新分镜板列表
        await loadStoryboardList();
      } else {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          toast.error(t('aiactoroEntry.messages.error.voiceDeleteFailed', { error: (error as Error).message }));
        }
      },
    });
  };
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.code === 0) {
          ...
          setConfiguredVoices(updatedConfigured);
          setAvailableVoices(updatedAvailable);
          toast.success(t('aiactoroEntry.messages.success.voiceApplied'));
        } else {
      ...
      toast.error(t('aiactoroEntry.messages.error.voiceApplyFailed', { error: (error as Error).message }));
    }
  };
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.code === 0) {
          // 更新成功，刷新音色列表
          await loadAllVoices();
          toast.success(t('aiactoroEntry.messages.success.voiceNameUpdated'));
        } else {
      ...
      toast.error(t('aiactoroEntry.messages.error.voiceNameUpdateFailed', { error: (error as Error).message }));
    } finally {
  ...
  const handleVoiceSelect = async (itemId: string, voiceId: string) => {
    ...
    try {
      ...
      const result = await response.json();
      if (result.code === 0) {
        toast.success(t('aiactoroEntry.messages.success.voiceBound'));
      } else {
        throw new Error(result.message || '音色绑定失败');
      }
    } catch (error) {
      console.error('音色绑定失败:', error);
      toast.error(t('aiactoroEntry.messages.error.voiceBindFailed', { error: (error as Error).message }));
    }
  };
  ...
  const handlePlayAudio = async (itemId: string) => {
    try {
      ...
      if (result.code === 0) {
        if (result.data?.audioUrl) {
          ...
          audio.play().catch((error) => {
            ...
            toast.error(t('aiactoroEntry.messages.success.audioPlaybackFailed'));
          });
        } else {
          toast.error(t('aiactoroEntry.messages.success.voiceStillGenerating'));
        }
      } else {
      ...
      toast.error(t('aiactoroEntry.messages.error.audioPlayError', { error: (error as Error).message }));
    }
  };
  ...
  const handleApplyBgm = async (bgm: any) => {
    ...
    if (!bgm.attachmentId) {
      toast.error(t('aiactoroEntry.userErrors.soundMissingAttachmentId'));
      return;
    }
    ...
    try {
      ...
      const result = await response.json();
      if (result.code === 0) {
        toast.success(t('aiactoroEntry.messages.success.soundApplied'));
        // 刷新音频内容列表
      ...
      toast.error(t('aiactoroEntry.messages.error.soundApplyFailed', { error: (error as Error).message }));
    }
  };
  ...
  const handleBgmSaveTimeEdit = async (bgmId: string) => {
    ...
    try {
      ...
      if (!bgmItem) {
        toast.error(t('aiactoroEntry.validation.soundNotFound'));
        return;
      }
  ...
  const handleImageGenerate = async () => {
    ...
    try {
      ...
      if (result.code === 0) {
        ...
        toast.success(t('aiactoroEntry.messages.success.imageGenerated'));
      } else {
        throw new Error(result.message || '图片生成失败');
      }
    } catch (error) {
      console.error('图片生成失败:', error);
      toast.error(t('aiactoroEntry.messages.error.imageGenerationFailed', { error: (error as Error).message }));
    } finally {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      const userStr = localStorage.getItem('user');
      if (!userStr) {
        toast.error(t('aiactoroEntry.userErrors.userInfoMissing'));
        return;
      }
      ...
      const userId = user.userId;
      if (!userId) {
        toast.error(t('aiactoroEntry.userErrors.userIdMissing'));
        return;
      }
  ...
  const handleVideoGenerate = async () => {
    if (!userInput.trim()) {
      toast.error(t('aiactoroEntry.validation.enterGeneratedContent'));
      return;
    }
    ...
    try {
      ...
      if (result.code === 0 && result.data) {
        ...
        toast.success(t('aiactoroEntry.messages.success.videoGenerationStarted'));
      ...
      toast.error(t('aiactoroEntry.messages.error.videoGenerationFailed', { error: (error as Error).message }));
      setGenerationStatus('');
      setIsGenerating(false);
  ...
  const handleBgmGenerate = async () => {
    ...
    try {
      ...
      if (!userId) {
        toast.error(t('aiactoroEntry.userErrors.userInfoIncomplete'));
        return;
      }
      ...
      toast.error(t('aiactoroEntry.messages.error.soundGenerationFailed', { error: (error as Error).message }));
    } finally {
      setIsGenerating(false);
      setGenerationStatus('');
    }
        ...
        try {
          ...
          if (detailResult.code === 0 && detailResult.data) {
            ...
            if (status === 'COMPLETED') {
              ...
              setIsGenerating(false);
              setGenerationStatus('');
              toast.error(t('aiactoroEntry.messages.error.videoGenerationRetry'));
    ...
    <div className="flex flex-col h-full bg-gradient-to-br from-blue-50 to-purple-50">
      <div className="flex flex-grow overflow-hidden">
          ...
          <div className="p-4 bg-white">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-base font-medium text-gray-900">{t('aiactoroEntry.sidebar.aiCreation')}</span>
              </div>
              ...
              <div >
                <Segmented
                  ...
                  options={[
                    { label: t('aiactoroEntry.tabs.script'), value: 'script' },
                    { label: t('aiactoroEntry.tabs.audio'), value: 'audio' },
                    { label: t('aiactoroEntry.tabs.image'), value: 'image' },
                    { label: t('aiactoroEntry.tabs.video'), value: 'video' }
                  ]}
            ...
            <div className={`bg-white rounded-lg h-full flex flex-col ${activeTab === 'script' ? '' : 'border border-gray-200'}`}>
                            ...
                            availableVoices.map((voice) => (
                              <div key={voice.voiceId} className="flex items-center space-x-3 py-3">
                                ...
                                <div className="w-8 h-8 rounded-full overflow-hidden flex items-center justify-center">
                                  {voice.voiceSource === 'CUSTOM' ? (
                                    ...
                                    <div className="w-full h-full bg-orange-500 flex items-center justify-center">
                                      <span className="text-xs text-white font-medium">{t('aiactoroEntry.status.systemVoice')}</span>
                                    </div>
                                  )}
                                </div>
                                <div className="flex space-x-2">
                                  ...
                                  >
                                    {t('aiactoroEntry.buttons.play')}
                                  </button>
                                  <button
                                    className="px-1 py-0.5 text-sm border border-blue-500 text-blue-500 rounded hover:bg-blue-50"
                                    onClick={() => handleApplyVoice(voice.voiceId)}
                                  >
                                    {t('aiactoroEntry.buttons.apply')}
                                  </button>
                                </div>
                              </div>
                            ))
                            ...
                            bgmList.map((bgm, index) => (
                              <div key={bgm.id || index} className="py-3 space-y-2">
                                <div className="flex items-start space-x-3">
                                  ...
                                  <div className="flex-1">
                                    <div className="text-sm font-medium text-gray-800">{bgm.prompt || bgm.name || bgm.title || t('aiactoroEntry.ui.defaultSoundName')}</div>
                                  </div>
                                  <button
                                ...
                                <div className="flex items-center space-x-2 pl-11">
                                    <span className="text-sm text-gray-600">{t('aiactoroEntry.preview.playbackLocation')}</span>
                                            ...
                                            <div className="w-20 bg-gray-200 overflow-hidden flex-shrink-0 relative" style={{ aspectRatio: '9 / 16' }}>
                                              ...
                                              <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-3 rounded-lg">
                                                <button
                                                  ...
                                                  title={t('aiactoroEntry.tooltips.viewOriginal')}
                                                >
                                                  <Icon icon="ri:external-link-line" className="w-3 h-3" />
                                                  {t('aiactoroEntry.preview.view')}
                                                </button>
                                                <button
                                                  onClick={() => {
                                                  ...
                                                  className="flex items-center gap-1 px-3 py-1.5 text-white text-xs font-medium hover:text-gray-200 transition-colors bg-transparent"
                                                  style={{ border: '1px solid #3E83F6', borderRadius: '4px' }}
                                                  title={t('aiactoroEntry.tooltips.applyImage')}
                                                >
                                            ...
                                            <div className="w-20 bg-gray-200 overflow-hidden flex-shrink-0 relative group" style={{ aspectRatio: '9 / 16' }}>
                                              ...
                                              <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-3 rounded-lg">
                                                <button
                                                  ...
                                                  className="flex items-center gap-1 px-3 py-1.5 text-white text-xs font-medium hover:text-gray-200 transition-colors bg-transparent"
                                                  style={{ border: '1px solid #3E83F6', borderRadius: '4px' }}
                                                  title={t('aiactoroEntry.tooltips.playVideo')}
                                                >
                                                ...
                                                <button
                                                  ...
                                                  className="flex items-center gap-1 px-3 py-1.5 text-white text-xs font-medium hover:text-gray-200 transition-colors bg-transparent"
                                                  style={{ border: '1px solid #3E83F6', borderRadius: '4px' }}
                                                  title={t('aiactoroEntry.tooltips.applyVideo')}
                                                >
                                                  <Icon icon="ri:check-line" className="w-3 h-3 text-green-400" />
                                                  {t('aiactoroEntry.buttons.apply')}
                                                </button>
              ...
              {activeTab === 'audio' ? (
                ...
                <BottomInputArea
                  ...
                  placeholder={t('aiactoroEntry.input.placeholder')}
                  generationStatus={generationStatus}
                  audioType={audioType}
                  voiceType={voiceType}
        ...
        <div className="flex-1 bg-gray-50 border-r border-gray-200 flex flex-col overflow-hidden">
          <SectionHeader
            title={
              activeTab === 'script' ? t('aiactoroEntry.tabs.script') :
              activeTab === 'audio' ? t('aiactoroEntry.tabs.audio') :
              activeTab === 'image' ? t('aiactoroEntry.tabs.image') : t('aiactoroEntry.tabs.video')
            }
            subtitle={
                        ...
                        <SortableAudioItem
                          ...
                          onSaveContentEdit={async (itemId) => {
                            try {
                              ...
                              if (!sceneId) {
                                toast.error(t('aiactoroEntry.validation.sceneNotFound'));
                                return;
                              }
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    if (cachedData && cachedData.downloadUrl) {
                      ...
                      toast.success(t('aiactoroEntry.messages.success.downloadStarted'));
                    } else {
                      toast.error(t('aiactoroEntry.messages.error.noVideoToDownload'));
                    }
                  }}
                >
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    if (cachedData && cachedData.lastFrame) {
                      ...
                      setQuestionTitle('');
                      setOptions([`${t('aiactoroEntry.ui.defaultOptionPrefix')}1`]);

                      toast.success(t('aiactoroEntry.messages.success.frameInserted'));
                      console.log('插入lastFrame:', cachedData.lastFrame);
                    } else {
                      toast.error(t('aiactoroEntry.ui.noFrameCacheHint'));
                    }
                  }}
                >
                      ...
                      {isLoadingPreviewVideo && (
                        <div className="absolute inset-0 bg-black/60 flex items-center justify-center z-20">
                          <div className="flex flex-col items-center">
                            ...
                            <div className="text-white text-sm">{t('aiactoroEntry.ui.creatingPreview')}</div>
                          </div>
                        </div>
                      )}
                        ...
                        {isEditorMode && (
                          <div className="absolute inset-0 bg-black/60 flex flex-col justify-end p-3 z-20">
                            ...
                            <div className="mt-3 flex items-center gap-2">
                              <button
                                onClick={() => {
                                  setOptions([...options, `${t('aiactoroEntry.ui.defaultOptionPrefix')}${options.length + 1}`]);
                                }}
                              ...
                              <button
                                onClick={async () => {
                                  try {
                                    ...
                                    if (result.code === 0) {
                                      toast.success(t('aiactoroEntry.messages.success.saved'));
                          ...
                          <div className="absolute bottom-0 left-0 right-0 h-8 bg-black/60 flex items-center justify-around backdrop-blur-sm">
                            <div className="text-center">
                              <div className="text-white text-sm">{t('aiactoroEntry.navigation.appName')}</div>
                            </div>
                            <div className="text-center">
                              <div className="text-white text-sm">{t('aiactoroEntry.navigation.myProfile')}</div>
                            </div>
                          </div>
                        </>
                      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-sm w-full mx-4 shadow-xl">
            <div className="flex items-center space-x-3 mb-4">
              ...
              <div>
                <h3 className="text-lg font-medium text-gray-900">{t('aiactoroEntry.dialogs.deleteConfirm.title')}</h3>
                <p className="text-sm text-gray-500">{t('aiactoroEntry.dialogs.deleteConfirm.message')}</p>
              </div>
            </div>
            <div className="flex space-x-3 justify-end">
              ...
              >
                {t('aiactoroEntry.dialogs.deleteConfirm.cancel')}
              </button>
              <button
              ...
              >
                {t('aiactoroEntry.dialogs.deleteConfirm.delete')}
              </button>
            </div>
          </div>
        </div>
...
export default AIActorEntryPage;
